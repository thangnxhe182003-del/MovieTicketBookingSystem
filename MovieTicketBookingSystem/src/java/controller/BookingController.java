...
public class BookingController extends HttpServlet {
    ...
    private void confirmBooking(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
                ...
                try {
                    ...
                    if (seat != null) {
                        ...
                        if ("VIP".equalsIgnoreCase(seat.getLoaiGhe())) {
                            multiplier = new java.math.BigDecimal("1.2");
                        } else if ("Couple".equalsIgnoreCase(seat.getLoaiGhe())) {
                            multiplier = new java.math.BigDecimal("1.5");
                        }
            ...
            for (java.math.BigDecimal p : originalTicketPrices) {
                ticketTotalOriginal = ticketTotalOriginal.add(p);
            }
                            ...
                            if ("Vé".equals(loai)) {
                                ...
                                for (int i = 0; i < originalTicketPrices.size(); i++) {
                                    if (isAdultList.get(i)) {
                                        adultTicketsSum = adultTicketsSum.add(originalTicketPrices.get(i));
                                    }
                                }
                                if (isPct) {
                                    ...
                                    if (discountAmount.compareTo(adultTicketsSum) > 0) {
                                        discountAmount = adultTicketsSum;
                                    }
                                }
                                if (max != null && discountAmount.compareTo(max) > 0) {
                                    discountAmount = max;
                                }
                            } else if ("Đồ ăn".equals(loai)) {
                                if (isPct) {
                                    discountAmount = productTotalPrice.multiply(val).divide(new java.math.BigDecimal("100"), 2, java.math.RoundingMode.HALF_UP);
                                } else if (isMoney) {
                                    discountAmount = val.min(productTotalPrice);
                                }
                            } else if ("Toàn đơn".equals(loai)) {
                                java.math.BigDecimal grand = ticketTotalOriginal.add(productTotalPrice);
                                if (isPct) {
                                    discountAmount = grand.multiply(val).divide(new java.math.BigDecimal("100"), 2, java.math.RoundingMode.HALF_UP);
                                } else if (isMoney) {
                                    discountAmount = val.min(grand);
                                }
                            }
                        }
                    }
            ...
            if (grandAfterDiscount.compareTo(java.math.BigDecimal.ZERO) < 0) {
                grandAfterDiscount = java.math.BigDecimal.ZERO;
            }
            // VAT after discount
                    ...
                    if (pct) {
                        ...
                        for (int i = 0; i < originalTicketPrices.size(); i++) {
                            ...
                            if (isAdultList.get(i)) {
                                ...
                                if (fin.compareTo(java.math.BigDecimal.ZERO) < 0) {
                                    fin = java.math.BigDecimal.ZERO;
                                }
                ...
                for (String seatId : selectedSeatIds) {
                    try {
                        ...
                        if (seat != null) {
                            ...
                            if (isAdult) {
                                adultSum = adultSum.add(giaVeCoSo.multiply(multiplier));
                            }
                            soLuongVe++;
                        }
                    } catch (NumberFormatException ignored) {
...
//                    java.math.BigDecimal adultSum = java.math.BigDecimal.ZERO;
                            ...
                            try {
                                ...
                                if (seat != null) {
                                    ...
                                    if (!"child".equalsIgnoreCase(age)) {
                                        ...
                                        if ("VIP".equalsIgnoreCase(seat.getLoaiGhe())) {
                                            mul = new java.math.BigDecimal("1.2");
                                        } else if ("Couple".equalsIgnoreCase(seat.getLoaiGhe())) {
                                            mul = new java.math.BigDecimal("1.5");
                                        }
    ...
    private int insertOrderAndDetails(model.Order order,
        ...
        int maOrder = orderDAO.createOrder(order);
        if (maOrder <= 0) {
            throw new IllegalStateException("Không tạo được đơn hàng");
        }
        // Xác thực tồn tại
        model.Order checkOrder = orderDAO.getOrderById(maOrder);
        if (checkOrder == null) {
            throw new IllegalStateException("Đơn hàng chưa tồn tại sau khi tạo");
        }
