...
public class OrderDAO extends DBContext {
    ...
    public int createOrder(Order order) {
        String sql = "INSERT INTO dbo.[Order] (MaKH, OrderCode, KenhDat, TrangThai, TongTien, GhiChu, CreatedAt, PaidAt) " +
    ...
    public List<Order> getPaidOrdersByCustomer(int maKH) {
        ...
        String sql = "SELECT * FROM dbo.[Order] WHERE MaKH = ? AND TrangThai = N'Đã thanh toán' ORDER BY CreatedAt DESC";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setInt(1, maKH);
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
