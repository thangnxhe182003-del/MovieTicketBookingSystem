...
<jsp:include page="../layout/admin-header.jsp">
    ...
    <jsp:param name="extraStyles" value="
        <style>
            ...
            .search-filter { background:#f8f9fa; padding:20px 30px; border-bottom:1px solid #e9ecef; display:flex; gap:20px; align-items:center; }
            .search-box { flex:1; max-width: 300px; }
            .search-box input, .filter-select { width:100%; padding:10px 12px; border:1px solid #ddd; border-radius:6px; font-size:14px; background:#fff; }
    ...
    <form class="search-filter" method="get" action="admin-tickets">
        <div style="width:220px;">
            <select class="filter-select" name="status">
                <option value="" ${empty filterStatus ? 'selected' : ''}>Mặc định: Thành công</option>
        ...
        <div style="width:220px;">
            <select class="filter-select" name="from">
                <option value="today" ${from == 'today' ? 'selected' : ''}>Từ hôm nay trở đi</option>
                <option value="all" ${from == 'all' ? 'selected' : ''}>Tất cả ngày</option>
            </select>
        </div>
        <button type="submit" class="btn-action" style="background:#e50914; color:#fff;">Lọc</button>
        <a href="admin-tickets" class="btn-action btn-reset">Reset</a>
    </form>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="status-badge 
                                    ${g.trangThai == 'Đã thanh toán' ? 'status-paid' : (g.trangThai == 'Đã sử dụng' ? 'status-used' : 'status-failed')}">${g.trangThai}</span>
                            </td>
                            <td>#${g.maOrder}</td>
                            <td>
                                <div class="action-buttons">
                                    <button type="button" class="btn-action btn-view" onclick="openDetails(${g.maOrder})"><i class="fas fa-eye"></i> View</button>
                                    <c:if test="${g.trangThai == 'Đã thanh toán'}">
